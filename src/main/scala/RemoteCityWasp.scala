...
object RemoteCityWasp {
  ...
  private case class RemoteSession(waspHttp: Http,
                                   config: Config,
      ...
      waspHttp(request("/lt/auth/") addCookie (sessionCookie) OK as.String)
        .flatMap {
          case Challenge(token) =>
            Future.successful(
              RemoteLoginChallenge(waspHttp, config, sessionCookie, token))
          case s =>
            Future.failed(
              new Error(s"Did not get login challenge token in $s."))
        }
    }
  }

  private case class RemoteLoginChallenge(waspHttp: Http,
                                          config: Config,
                                          sessionCookie: Cookie,
                                          challenge: String)
    ...
    def login = {
      ...
      waspHttp(
        request("/lt/auth/login/") << credentials addCookie (sessionCookie))
        ...
        .flatMap {
          ...
          case Some("/lt/auth/") =>
            Future.failed(
              new Error(s"Unable to log in. Check username/password."))
          case _ => Future.failed(new Error(s"Error while logging in."))
        }
    }
  }

  private case class RemoteLoggedIn(waspHttp: Http,
                                    config: Config,
                                    sessionCookie: Cookie)
      extends LoggedIn
      with Common {
    def currentCar = {
      ...
      waspHttp(
        request / "mobile" / "lt" / "reservation" / "active" addCookie (sessionCookie) OK as.String)
        .flatMap {
          case CarReserved(msLeft, carId) =>
            Future.successful(
              Some(RemoteLockedCar(waspHttp, config, sessionCookie, carId)))
          case CarUnlocked(carId) =>
            Future.successful(
              Some(RemoteUnlockedCar(waspHttp, config, sessionCookie, carId)))
    ...
    def parkedCars = {
      ...
      waspHttp(
        request / "mobile" / "lt" / "" addCookie (sessionCookie) OK as.String)
        .map(resp =>
          CarListing.findAllMatchIn(resp).toSeq.map { m =>
            ParkedCar(m.group(1).toInt,
                      m.group(2),
                      m.group(3),
                      m.group(4),
                      m.group(5).toDouble,
  ...
  private case class RemoteLockedCar(waspHttp: Http,
                                     config: Config,
                                     sessionCookie: Cookie,
                                     carId: String)
      extends LockedCar
    ...
    def unlock =
      waspHttp(
        request / "mobile" / "lt" / "reservation" / "start" / carId addCookie (sessionCookie))
  ...
  private case class RemoteUnlockedCar(waspHttp: Http,
                                       config: Config,
                                       sessionCookie: Cookie,
                                       carId: String)
      extends UnlockedCar
    ...
    def lock =
      waspHttp(
        request / "mobile" / "lt" / "reservation" / "stop" / carId addCookie (sessionCookie))
        .map(_.header("Location").headOption)
        .flatMap {
