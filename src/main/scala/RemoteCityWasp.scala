...
object RemoteCityWasp {
  ...
  case class RemoteCityWasp(config: Config) extends CityWasp with Common {
    val waspHttp = Http.configure { _
      .setAllowPoolingConnection(config.getBoolean("http.connection-pooling"))
  ...
  private case class RemoteLoginChallenge(waspHttp: Http, config: Config, sessionCookie: Cookie, challenge: String) extends LoginChallenge with Common {
    def login = {
      val credentials = Map(
      ...
      waspHttp(request("/lt/auth/login/") << credentials addCookie(sessionCookie)).map(_.header("Location").headOption).flatMap {
  ...
  private case class RemoteLoggedIn(waspHttp: Http, config: Config, sessionCookie: Cookie) extends LoggedIn with Common {
    def currentCar = {
      ...
      waspHttp(request / "lt" / "reservation" / "active" addCookie(sessionCookie) OK as.String).flatMap {
        case CarReserved(msLeft, carId) => Future.successful(Some(RemoteLockedCar(waspHttp, config, sessionCookie, carId)))
  ...
  private case class RemoteLockedCar(waspHttp: Http, config: Config, sessionCookie: Cookie, carId: String) extends LockedCar with Common {
    def unlock =
  ...
  private case class RemoteUnlockedCar(waspHttp: Http, config: Config, sessionCookie: Cookie, carId: String) extends UnlockedCar with Common {
    def lock =
      waspHttp(request / "lt" / "car" / "lock" / carId addCookie(sessionCookie)).map(_.header("Location").headOption).flatMap {
